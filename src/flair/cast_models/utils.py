...
def segment_mean(x, index):
    ...
    """
    x = x.view(-1, x.shape[-1]) # (BL, C)
    index = index.view(-1) # (BL)
...
def segment_mean_nd(x, index, start_dim=-3, end_dim=-2):
    ...
    # Flatten tensors.
    index = index.flatten(start_dim, end_dim).flatten(0, start_dim-1) # (B, L)
    x = x.flatten(start_dim, end_dim).flatten(0, start_dim-1) # (B, L, C)

    # Separate label index by batch.
    batch_inds = torch.arange(0, x.shape[0],
                              dtype=index.dtype, device=index.device) # (B)
    max_ind = index.max() + 1 # 196
    index = batch_inds.view(-1, 1) * max_ind + index # (B, 1) + (B, L) -> (B, L)
    mean_x = segment_mean(x.contiguous(), index) # (_, C), _ maybe B*L

    # If the minimum label index of the last batch is less than
    # `max_ind`, the averaged `x` would be shorter than expected.
    max_ind = 324 if max_ind > 300 else 196 # hard coding
...
def segment_max(x, index):
    ...
    max_x = torch.zeros((max_index, x.shape[-1]),
                        dtype=x.dtype,
                        device=x.device)
    max_x = max_x.scatter_reduce(0, index_2d, x, reduce='amax', include_self=True)
    
    out = torch.cat([mean_x, max_x], dim=-1) 

def segment_mean_max(x, index):
    """Function as tf.segment_mean.
    """
    x = x.view(-1, x.shape[-1]) # (BL, C)
    index = index.view(-1) # (BL)
...
def segment_mean_max_nd(x, index, start_dim=-3, end_dim=-2):
    """Function segment_mean for nd-array as inputs.
    ...
    max_ind = index.max() + 1 # 196
    index = batch_inds.view(-1, 1) * max_ind + index # (B, 1) + (B, L) -> (B, L)
    mean_max_x = segment_mean_max(x.contiguous(), index) # (_, C), _ maybe B*L

    # If the minimum label index of the last batch is less than
    # `max_ind`, the averaged `x` would be shorter than expected.
    expected_n = (batch_inds.max() + 1) * max_ind
    if mean_max_x.shape[0] < expected_n:
        mean_max_x = F.pad(mean_max_x, (0, 0, 0, expected_n - mean_max_x.shape[0]),
                       'constant', 0)
    mean_max_x = mean_max_x.view(batch_sizes + [max_ind] + [channel *2])
