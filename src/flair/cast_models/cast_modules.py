...
class Pooling(nn.Module):
    ...
    def forward(self, cls_token, x, padding_mask=None, num_clusters=None):
        ...
        cls_token, centroid, pool_logit, sampled_x_inds = self.pool_block(
            cls_token=cls_token, src=x, mask=padding_mask, num_clusters=num_clusters)
...
class PoolingWoCls(nn.Module):

    def __init__(self, pool_block):
        super(PoolingWoCls, self).__init__()
        self.pool_block = pool_block

    def forward(self, x, padding_mask=None, num_clusters=None):
        x, attn_up, attn_down = self.pool_block(
             src=x, mask=padding_mask, num_clusters=num_clusters)

        pool_padding_mask = torch.zeros(
            (attn_up.shape[0], attn_up.shape[1]),
            dtype=torch.bool,
            device=attn_up.device)

        return x, attn_up, attn_down, pool_padding_mask
...
class BlockFusionv2(nn.Module):

    def __init__(self, dim, num_stages, last_module):
        super(BlockFusionv2, self).__init__()

        self.last_module = last_module
        self.num_stages = num_stages
        self._discrete = True
        if last_module=="graph_pooling":
            self.proj_blocks = self._make_proj_block(dim * num_stages, dim)
        elif last_module=="vit":
            self.proj_blocks = self._make_proj_block(dim * (num_stages+1), dim)
    ...
    def forward(self, intermediates):
        ...
        if self._discrete:
            for i in range(1, self.num_stages + 1):
                labels[f"label{i}_{i+1}"] = torch.argmax(intermediates[f"logit{i}"], dim=-1)

            for i in range(1, self.num_stages):
                labels[f"label1_{i+2}"] = torch.gather(labels[f"label{i+1}_{i+2}"], 1, labels[f"label1_{i+1}"])
        else:
            raise NotImplementedError("Only support discrete unpooling")

        # get blocks
        blocks = list()
        blocks.append(intermediates["block1"]) # "block1"
        for i in range(2, self.num_stages + 1):
            blocks.append(self._unpool(intermediates[f"block{i}"], labels[f"label1_{i}"]))

        # get cls_tokens
        cls_tokens = [intermediates[f"cls_token{i}"] for i in range(1, self.num_stages + 1)]

        if self.last_module == 'vit':
            blocks.append(self._unpool(intermediates[f"block{self.num_stages+1}"], labels[f"label1_{self.num_stages+1}"]))
            cls_tokens.append(intermediates[f"cls_token{self.num_stages+1}"])

        # fuse blocks
        out_block = torch.cat(blocks, dim=-1)
        out_cls_token = torch.cat(cls_tokens, dim=-1)    
