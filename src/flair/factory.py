...
from .model import convert_weights_to_lp, convert_to_custom_text_state_dict, \
    resize_pos_embed, get_cast_dtype, resize_text_pos_embed, set_model_preprocess_cfg, FLAIR, CAFT
...
from .loss import FlairLoss, HieFlairLoss
...
def create_model(
    ...
    if pretrained and pretrained.lower() == 'openai':
        ...
        if "CAFT" in model_name:
            model = CAFT(**model_cfg, cast_dtype=cast_dtype)
...
def create_loss(args):
    ...
    assert (args.use_flair_loss and args.hieflair_loss == "disabled") or \
            ((not args.use_flair_loss) and args.hieflair_loss != "disabled")
    ...
    if args.hieflair_loss != "disabled":
        return HieFlairLoss(rank=args.rank,
                            world_size=args.world_size,
                            num_cap_per_img=args.num_sampled_captions,
                            loss_type = args.hieflair_loss,
