...
def parse_args(args):
    ...
    parser.add_argument(
        "--inference-mode",
        choices=["clip", "flair", "caft", "caft_topk"],
        default="caft",
        help=(
            "Which inference/retrieval codepath to use: original CLIP-style, FLAIR, CAFT "
            "(full alpha-blended coarse+fine score over every image-text pair), or "
            "'caft_topk' (coarse matching shortlists each image's own top --topk candidate "
            "texts, then only those get the fine rerank)."
        )
    )
    parser.add_argument(
        "--topk",
        type=int,
        default=16,
        help="K texts per image to keep when --inference-mode caft_topk is used."
    )
    parser.add_argument(
        "--fixed-merged-num",
        default=False,
    ...
    parser.add_argument(
        "--hieflair-loss",
        choices=["ftcs_cmps", "ftcs_ctcs", "ftcs_fmps_cmps", "ftcs_fmps_ctcs", "cmps", "ctcs", "ctcs_cmps"],
        default="ftcs_fmps_cmps",
        help="Whether to use the hierarchical text-conditioned sigmoid loss or not."
    )
    parser.add_argument(
        "--add-mps-loss",
        default=False,
    ...
    parser.add_argument(
        ...
        default=0.0,
        help=(
            "Weight for blending coarse and fine (mid-level) retrieval similarity in "
            "compute_similarity_caft: final_score = (1 - alpha) * coarse + alpha * fine. "
            "alpha=0.0 (default) uses coarse-only (skips fine computation entirely); "
            "alpha=1.0 uses fine-only."
        )
    )
    ...
    parser.add_argument(
        "--eval-img-batch-size", type=int, default=128,
        help=(
            "Image batch size for retrieval eval dataloaders (COCO/Flickr/DOCCI/Urban-1k/"
            "IIW/DCI/ShareGPT4V/cc3m-train), decoupled from --batch-size (which also drives "
            "training)."
        )
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        choices=["diverse_sampling", "caft_sampling"],
        default="caft_sampling",
        help="Floating point precision."
    )
    parser.add_argument(
        "--negative-type",
