...
from .transformer import LayerNormFp32, LayerNorm, QuickGELU, Attention, VisionTransformer, TextTransformer, \
    text_global_pool, \
    PureAttentionPoolingBlock, VisionPostProcess, TextPostProcess, ParagraphTransformer
from open_clip.utils import to_2tuple
from .cast_models.castv2 import CAST
from .cast_models.basic_modules import ConvStemV1
from timm.models.vision_transformer import _cfg


@dataclass
class CAST_CLIPVisionCfg:
    cast_model_name: str = 'CAST'
    patch_size: int = 8
    image_size: Union[Tuple[int, int], int] = 224
    embed_dim: int = 768
    num_clusters: list = field(default_factory=lambda: [64, 32, 16, 1])
    depth: list = field(default_factory=lambda: [3, 3, 3, 3])

    patchify_stem: str = 'patch'
    global_pool: str = "graph_pool"
    ...
    fill_dead_tokens: bool = True
    hierarchy_flair: bool = False
...
def _build_vision_tower(
    ...
    if isinstance(vision_cfg, dict):
        if 'cast_model_name' in vision_cfg:
            is_CAST = True
            vision_cfg = CAST_CLIPVisionCfg(**vision_cfg)
        else:
    ...
    if vision_cfg.timm_model_name:
        ...
        if vision_cfg.patchify_stem == 'ConvStemV1':
            embed_layer = ConvStemV1
        else:
            raise NotImplementedError("Unkown patchify_stem.")
        visual = CAST(
            img_size=[vision_cfg.image_size],
            patch_size=vision_cfg.patch_size,
            reg_tokens=vision_cfg.reg_tokens,
            embed_dim=vision_cfg.embed_dim,
            output_dim=embed_dim,
            num_clusters=vision_cfg.num_clusters,
            ...
            fill_dead_tokens=vision_cfg.fill_dead_tokens,
            text_con=text_con,
            hierarchy_flair=vision_cfg.hierarchy_flair
        )
        visual.default_cfg = _cfg()
    else:
...
class FLAIR(nn.Module):
    ...
    def get_text_last_attn(self, text, average_attn_weights=True):
        txt_dim = text.shape[-1]
        cast_dtype = self.transformer.get_cast_dtype()
        text = text.view(-1, txt_dim)  #(B*K, 77)
        x = self.token_embedding(text).to(cast_dtype)  # [batch_size, n_ctx, d_model]
        x = x + self.positional_embedding.to(cast_dtype)
        x = self.transformer.forward_with_attn(x, attn_mask=self.attn_mask, average_attn_weights=average_attn_weights)
        return self.transformer.get_last_attn()
...
class CAFT(FLAIR):
    def __init__(
            ...
            vision_cfg: CLIPVisionCfg,
            text_cfg: CLIPTextCfg,
            text_coarse_cfg: CLIPTextCfg, # added
            paragraph_k: int = 4, # added
            use_coarse_tcs: bool = False, # added
            quick_gelu: bool = False,
            init_logit_scale: float = np.log(1 / 0.07),
            init_logit_bias: Optional[float] = None,
            cast_dtype: Optional[torch.dtype] = None,
            output_dict: bool = False,
            pretrained_fine_path=None
    ):
        super().__init__(embed_dim, vision_cfg, text_cfg, quick_gelu, init_logit_scale, init_logit_bias, cast_dtype, output_dict)
        
        if use_coarse_tcs:
            self.visual_proj_coarse = PureAttentionPoolingBlock(context_dim=embed_dim)
        else:
            self.visual_proj_coarse = None
        self.paragraph_encoder = ParagraphTransformer(embed_dim, text_coarse_cfg)
        self.paragraph_k = paragraph_k
        
        self.logit_scale_coarse = nn.Parameter(torch.ones([]) * init_logit_scale)
        if init_logit_bias is not None:
            self.logit_bias_coarse = nn.Parameter(torch.ones([]) * init_logit_bias)
        else:
            self.logit_bias_coarse = None

        self.image_post_coarse = VisionPostProcess(context_dim=self.visual.width,
                                            output_dim=embed_dim,
                                            normalize_final=False,
                                            skip_ln=True)
        self.text_post_coarse = TextPostProcess(context_dim=self.paragraph_encoder.width,
                                         output_dim=embed_dim,
                                         normalize_final=False,
                                         skip_ln=True)

        self.logit_scale_hn = None
        # self.logit_scale_hn = nn.Parameter(torch.ones([]) * init_logit_scale)

    def encode_image(self, image, normalize: bool = False):
        image_fine_local, image_fine_global, image_coarse_local, image_coarse_global  = self.visual(image)
        return image_fine_local, image_fine_global, image_coarse_local, image_coarse_global

    def encode_text_coarse(self, x, k=8, normalize: bool = False, hard_negative=False):
        """
        x: (B*k, C) or (B, k, C)
        return: (B, C)
        """
        x_dim = x.shape[-1]
        x = x.reshape(-1, k, x_dim) # (B, k, C)
        x = x[:, :self.paragraph_k]
        if hard_negative:
            x = self._hard_neg_sampling(x, num_negative=16) # (B, k, C) -> (B*2, K, C )
        return self.paragraph_encoder(x) # (B, paragraph_k, C) -> (B, C)
    ...
    def forward(
        ...
        image_fine_local, image_fine_global, image_coarse_local, image_coarse_global = self.encode_image(image) #(B, 80, C), (B, 30, C)
        text_fine_global, _ = self.encode_text(text) # (B*K, C)
        text_coarse_global = self.encode_text_coarse(text_fine_global, k=text.shape[1], hard_negative=hard_negative)
        text_fine_global, text_coarse_global = self.text_post(text_fine_global), self.text_post_coarse(text_coarse_global)
        image_fine_local, image_fine_global = self.image_post(image_fine_local), self.image_post(image_fine_global)
        image_coarse_local, image_coarse_global = self.image_post_coarse(image_coarse_local), self.image_post_coarse(image_coarse_global)
        ...
        out_dict = {
            ...
            "text_fine_global": text_fine_global,
            "text_coarse_global": text_coarse_global,
            "logit_scale": self.logit_scale.exp(),
            "logit_scale_coarse": self.logit_scale_coarse.exp(),
            "visual_proj": self.visual_proj
        }
        if self.logit_bias is not None:
            out_dict['logit_bias'] = self.logit_bias
        if self.logit_bias_coarse is not None:
            out_dict['logit_bias_coarse'] = self.logit_bias_coarse
        if self.visual_proj_coarse is not None:
            out_dict['visual_proj_coarse'] = self.visual_proj_coarse
        if self.logit_scale_hn is not None:
            out_dict['logit_scale_hn'] = self.logit_scale_hn.exp()
        return out_dict