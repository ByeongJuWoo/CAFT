...
def poolfeatv2(input, prob, fold, h=16, w=16,):
    """
    - input: (B, C=384, H=768, W=768)
    - prob : (B, 9, H, W)
    - h, w: 
    """
    B, C, H, W = input.shape
    sh, sw = H//h, W//w
    input = input.reshape(B, C, sh, h, sw, w).permute(0, 2, 4, 3, 5, 1) \
        .reshape(B, sh*sw, h*w, C) 
    prob = prob.reshape(B, 9, sh, h, sw, w).permute(0, 2, 4, 3, 5, 1) \
        .reshape(B, sh*sw, h*w, 9)
    prob_sum = prob.sum(2).transpose(1, 2).reshape(B, 9, sh, sw)
    prob_sum = fold(prob_sum) # (B, 1, sh, sw)

    pooledfeat = input.transpose(-1, -2) @ prob # (B, sh*sw, C, 9)
    pooledfeat = fold(pooledfeat.permute(0, 2, 3, 1).reshape(B*C, 9, sh, sw).reshape(B, C, sh, sw))
    pooledfeat = pooledfeat/prob_sum
...
#     feat_ = torch.cat([
#         input,
#         torch.ones((b, 1, h, w), device=input.device, dtype=input.dtype)
#     ], dim=1)

#     h_shift_unit = 1
#     w_shift_unit = 1
#     p2d = (w_shift_unit, w_shift_unit, h_shift_unit, h_shift_unit)  # (left, right, top, bottom)
#     pooled_h = h // sp_h
#     pooled_w = w // sp_w

#     offset_slices = [
#         (slice(2*h_shift_unit, pooled_h), slice(2*w_shift_unit, pooled_w)),      # top-left
#         (slice(2*h_shift_unit, pooled_h), slice(w_shift_unit, pooled_w-w_shift_unit)),   # top
#         (slice(2*h_shift_unit, pooled_h), slice(0, pooled_w-2*w_shift_unit)),         # top-right
#         (slice(h_shift_unit, pooled_h-h_shift_unit), slice(2*w_shift_unit, pooled_w)),   # left
#         (slice(h_shift_unit, pooled_h-h_shift_unit), slice(w_shift_unit, pooled_w-w_shift_unit)), # center
#         (slice(h_shift_unit, pooled_h-h_shift_unit), slice(0, pooled_w-2*w_shift_unit)),       # right
#         (slice(0, pooled_h-2*h_shift_unit), slice(2*w_shift_unit, pooled_w)),             # bottom-left
#         (slice(0, pooled_h-2*h_shift_unit), slice(w_shift_unit, pooled_w-w_shift_unit)),       # bottom
#         (slice(0, pooled_h-2*h_shift_unit), slice(0, pooled_w-2*w_shift_unit))              # bottom-right
#     ]

#     feat_sum = torch.zeros((b, c, pooled_h, pooled_w), device=input.device, dtype=input.dtype)
#     prob_sum = torch.zeros((b, 1, pooled_h, pooled_w), device=input.device, dtype=input.dtype)

#     for i, (vs, hs) in enumerate(offset_slices):
#         weighted = feat_ * prob[:, i:i+1, :, :]               # (B, C+1, H, W)
#         pooled = F.avg_pool2d(weighted, kernel_size=(sp_h, sp_w), stride=(sp_h, sp_w))
#         padded = F.pad(pooled, p2d, mode='constant', value=0) # (B, C+1, H, W) 
#         feat_sum[:, :, vs, hs] += padded[:, :-1, vs, hs]      # 
#         prob_sum[:, :, vs, hs] += padded[:,  -1:, vs, hs]     # 

#     pooled_feat = feat_sum / (prob_sum + 1e-8)
#     return pooled_feat

def upfeat(input, prob, up_h=2, up_w=2):
    # input b*n*H*W  downsampled [B, ]
    # prob b*9*h*w
    b, c, h, w = input.shape
...
def update_spixl_map (spixl_map_idx_in, assig_map_in):
    ...
    # assig_max,_ = torch.max(assig_map, dim=1, keepdim= True) # (B, 9, H, W)
    # assignment_ = torch.where(assig_map == assig_max, torch.ones(assig_map.shape) \
    #                           .cuda(),torch.zeros(assig_map.shape).cuda()) #
    assignment_ = F.one_hot(torch.argmax(assig_map, dim=1), num_classes=assig_map.shape[1]).permute(0, 3, 1, 2)
