...
def test(args, model, img_paths, save_path, idx):
    ...
    spixel_viz, spixel_label_map = get_spixel_image((ori_img + mean_values).clamp(0, 1), ori_sz_spixel_map.squeeze(), n_spixels= n_spixel,  b_enforce_connect=False)
    ...
    print(f"{spixl_save_name} is saved")
    img_to_save = (spixel_viz * 255).clip(0, 255).astype(np.uint8)
    imwrite(spixl_save_name, img_to_save.transpose(1, 2, 0))
...
def main():
    ...
    # create model
    network_data = torch.load(args.pretrained, map_location="cuda:0")
